...
class Evaluator:
    ...
    @staticmethod
    def _compute_distmat(g_feats, q_feats, bs=256):
        mats = []
    ...
    # @staticmethod
    # def _compute_distmat(g_feats, q_feats, squared=True):
    #     half = q_feats.shape[0] // 2

    #     g_feats = tf.convert_to_tensor(value=g_feats) # shape [num_gallery, 2048]
    #     q_feats = tf.convert_to_tensor(value=q_feats)  # shape [num_query, 2048]
    #     g_feats = tf.expand_dims(g_feats, 0)
    #     q_feats = tf.expand_dims(q_feats, 1)

    #     tmp = tf.subtract(g_feats, q_feats[:half, :, :])
    #     dist1 = tf.norm(tmp, axis=2)

    #     tmp = tf.subtract(g_feats, q_feats[half:, :, :])
    #     dist2 = tf.norm(tmp, axis=2)

    #     dist = tf.concat([dist1, dist2], axis=0)
    #     return dist
